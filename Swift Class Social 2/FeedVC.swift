...
class FeedVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var postField: FancyTextField!
    ...
    @IBAction func postBtnTapped(_ sender: Any) {
        ...
        guard let caption = postField.text, caption != "" else {
            print("ISAAC: Caption must be entered")
            return
        }
        guard let img = imageAdd.image, imageSelected == true else {
            print("ISAAC: An image must be selected")
            return
        }
        ...
        if let imgData = UIImageJPEGRepresentation(img, 0.2) {
            ...
            DataService.ds.REF_POST_IMAGES.child(imgUid).put(imgData, metadata: metadata) {(metadata, error) in
                if error != nil {
                    print("ISAAC: Unable to upload image to Firebase Storage")
                } else {
                    print("ISAAC: Successfully uploaded image to Firebase Storage")
